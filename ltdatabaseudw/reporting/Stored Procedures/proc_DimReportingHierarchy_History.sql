CREATE PROC [reporting].[proc_DimReportingHierarchy_History] @DivisionList [VARCHAR](8000),@SubdivisionList [VARCHAR](8000),@DepartmentMinDimReportingHierarchyKeyList [VARCHAR](8000),@DimReportingHierarchyKeyList [VARCHAR](8000),@StartMonthStartingDimDateKey [VARCHAR](32),@EndMonthEndingDimDateKey [VARCHAR](32) AS
BEGIN 
SET XACT_ABORT ON
SET NOCOUNT ON
...
----- "fnOperationsDimReportingHierarchy";  This is to be called by a reporting stored procedure
----- which needs a "History" instance of the reporting hierarchy



----- Execution Sample
--- Exec [reporting].[proc_DimReportingHierarchy_History] 'All Divisions','All Subdivisions','All Departments','N/A','20181001','20181031'
-----
...
SET @ReportRegionType = (Select CASE WHEN COUNT(DISTINCT RegionType) = 1 
 ...
 ----- In answer to the question of how to set up a stored procedure to replace the functions used in LTFDM.  In this case, replacing "fnOperationsDimReportingHierarchy" with an "Active" version stored procedure
