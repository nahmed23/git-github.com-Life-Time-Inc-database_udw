CREATE PROC [reporting].[proc_AnnualRevenueClubSummaryByReportingDepartmentAndProductGroup] @EndFourDigitYearDashTwoDigitMonth [CHAR](7),@DimReportingHierarchyKeyList [VARCHAR](8000),@DimClubIDList [VARCHAR](4000),@SalesSourceList [VARCHAR](4000),@TotalReportingHierarchyKeyCount [INT] AS
BEGIN 
SET XACT_ABORT ON
SET NOCOUNT ON
...
IF 1=0 BEGIN
 ...
 ----- Execution Sample
 ------ exec [reporting].[proc_AnnualRevenueClubSummaryByReportingDepartmentAndProductGroup] '2015-12','All Product Groups','151|-1','MMS|Hybris|Cafe',30
 -----
 ----- This stored procedure is used by Report ID 108 - Annual Revenue Club Summary
...
SELECT @SSSGGrandOpeningDeadlineDate = PriorYearPromptDimDate.calendar_date,
 ...
 WHERE PromptDimDate.four_digit_year_dash_two_digit_month = @EndFourDigitYearDashTwoDigitMonth
...
SET @EComm60DayChallengeRevenueStartMonthStartDimDateKey = (SELECT CASE WHEN (Select month_number_in_year 
				   ...
				   WHERE dim_date_key = @StartMonthStartingDimDateKey) in (2,5,8,11)
			THEN (Select prior_month_starting_dim_date_key
			        FROM [marketing].[v_dim_date]
			        WHERE dim_date_key = @StartMonthStartingDimDateKey)
            ELSE (Select month_starting_dim_date_key
                    FROM [marketing].[v_dim_date]
				   WHERE dim_date_key = @StartMonthStartingDimDateKey)
			END 
            FROM [marketing].[v_dim_date]
            WHERE dim_date_key = @StartMonthStartingDimDateKey)  ---- to limit result set to one record
...
SET @EComm60DayChallengeRevenueEndMonthEndDimDateKey = (SELECT CASE WHEN (Select month_number_in_year 
                    FROM [marketing].[v_dim_date] 
				   WHERE dim_date_key = @EndMonthEndingDimDateKey) in (1,4,7,10)
			THEN (Select prior_month_ending_dim_date_key
			        FROM [marketing].[v_dim_date] 
			        WHERE dim_date_key = @EndMonthEndingDimDateKey)
            ELSE (Select month_ending_dim_date_key
                    FROM [marketing].[v_dim_date] 
				   WHERE dim_date_key = @EndMonthEndingDimDateKey)
			END 
            FROM [marketing].[v_dim_date]
            WHERE dim_date_key = @EndMonthEndingDimDateKey)  ---- to limit result set to one record
...
SELECT DimClub.dim_club_key AS DimClubKey,     ------- new name
  ...
  LEFT JOIN [marketing].[v_dim_description] PTRCLArea
   ON PTRCLArea.dim_description_key = DimClub.pt_rcl_area_dim_description_key
  LEFT JOIN [marketing].[v_dim_description] MemberActivitiesRegion
...
SELECT #DimReportingHierarchy.DimReportingHierarchyKey,
       ...
       FactAllocatedRevenue.allocated_dim_club_key AS DimClubKey,        -------- New Name
	   ...
	   #DimLocationInfo.club_open_dim_date_key
...
SELECT #DimReportingHierarchy.DimReportingHierarchyKey,
       ...
       #DimReportingHierarchy.DepartmentName AS RevenueReportingDepartment,
       #DimReportingHierarchy.ProductGroupName AS RevenueProductGroup,
       FactAllocatedRevenue.allocated_dim_club_key AS DimClubKey,
       RevenuePostingMonthDimDate.month_number_in_year AS RevenueMonth,
       RevenuePostingMonthDimDate.year AS RevenueYear,
       FactAllocatedRevenue.allocated_amount AS RevenueAmount,
	   #DimLocationInfo.club_open_dim_date_key
...
SELECT DimClubKey,
 ...
 WHERE club_open_dim_date_key <= @SSSGGrandOpeningDeadlineDateKey
 GROUP BY DimClubKey,
          DimReportingHierarchyKey,
          DepartmentMinDimReportingHierarchyKey,
          RevenueMonth
...
--Goals
SELECT FactGoal.dim_club_key AS DimClubKey,
...
--Put it all together
SELECT ISNULL(#ProductGroupGoalSummary.DimClubKey,#ProductGroupRevenueSummary.DimClubKey) DimClubKey,
       ISNULL(#ProductGroupGoalSummary.RevenueReportingDepartment,#ProductGroupRevenueSummary.RevenueReportingDepartment) RevenueReportingDepartment,
  ...
  FULL OUTER JOIN #ProductGroupRevenueSummary
    ON #ProductGroupGoalSummary.DimClubKey = #ProductGroupRevenueSummary.DimClubKey
   AND #ProductGroupGoalSummary.DimReportingHierarchyKey = #ProductGroupRevenueSummary.DimReportingHierarchyKey
  ...
  LEFT JOIN #ProductGroupSSSGPriorRevenueSummary
    ON ISNULL(#ProductGroupGoalSummary.DimClubKey,#ProductGroupRevenueSummary.DimClubKey) = #ProductGroupSSSGPriorRevenueSummary.DimClubKey
   AND ISNULL(#ProductGroupGoalSummary.DimReportingHierarchyKey,#ProductGroupRevenueSummary.DimReportingHierarchyKey) = #ProductGroupSSSGPriorRevenueSummary.DimReportingHierarchyKey
  ...
  LEFT JOIN #ProductGroupSSSGPromptRevenueSummary
    ON ISNULL(#ProductGroupGoalSummary.DimClubKey,#ProductGroupRevenueSummary.DimClubKey) = #ProductGroupSSSGPromptRevenueSummary.DimClubKey
   AND ISNULL(#ProductGroupGoalSummary.DimReportingHierarchyKey,#ProductGroupRevenueSummary.DimReportingHierarchyKey) = #ProductGroupSSSGPromptRevenueSummary.DimReportingHierarchyKey
  ...
  LEFT JOIN #ProductGroupYOYSummary
    ON ISNULL(#ProductGroupGoalSummary.DimClubKey,#ProductGroupRevenueSummary.DimClubKey) = #ProductGroupYOYSummary.DimClubKey
   AND ISNULL(#ProductGroupGoalSummary.DimReportingHierarchyKey,#ProductGroupRevenueSummary.DimReportingHierarchyKey) = #ProductGroupYOYSummary.DimReportingHierarchyKey
 ...
 GROUP BY ISNULL(#ProductGroupGoalSummary.DimClubKey,#ProductGroupRevenueSummary.DimClubKey),
          ISNULL(#ProductGroupGoalSummary.RevenueReportingDepartment,#ProductGroupRevenueSummary.RevenueReportingDepartment),
...
SELECT ISNULL(#DepartmentGoalSummary.DimClubKey,#DepartmentRevenueSummary.DimClubKey) DimClubKey,
       ISNULL(#DepartmentGoalSummary.RevenueReportingDepartment,#DepartmentRevenueSummary.RevenueReportingDepartment) RevenueReportingDepartment,
  ...
  FULL OUTER JOIN #DepartmentRevenueSummary
    ON #DepartmentGoalSummary.DimClubKey = #DepartmentRevenueSummary.DimClubKey
   AND #DepartmentGoalSummary.DepartmentMinDimReportingHierarchyKey = #DepartmentRevenueSummary.DepartmentMinDimReportingHierarchyKey
  ...
  LEFT JOIN #DepartmentSSSGPriorRevenueSummary
    ON ISNULL(#DepartmentGoalSummary.DimClubKey,#DepartmentRevenueSummary.DimClubkey) = #DepartmentSSSGPriorRevenueSummary.DimClubKey
   AND ISNULL(#DepartmentGoalSummary.DepartmentMinDimReportingHierarchyKey,#DepartmentRevenueSummary.DepartmentMinDimReportingHierarchyKey) = #DepartmentSSSGPriorRevenueSummary.DepartmentMinDimReportingHierarchyKey
  ...
  LEFT JOIN #DepartmentSSSGPromptRevenueSummary
    ON ISNULL(#DepartmentGoalSummary.DimClubKey,#DepartmentRevenueSummary.DimClubkey) = #DepartmentSSSGPromptRevenueSummary.DimClubKey
   AND ISNULL(#DepartmentGoalSummary.DepartmentMinDimReportingHierarchyKey,#DepartmentRevenueSummary.DepartmentMinDimReportingHierarchyKey) = #DepartmentSSSGPromptRevenueSummary.DepartmentMinDimReportingHierarchyKey
  ...
  LEFT JOIN #DepartmentYOYSummary
    ON ISNULL(#DepartmentGoalSummary.DimClubKey,#DepartmentRevenueSummary.DimClubkey) = #DepartmentYOYSummary.DimClubKey
   AND ISNULL(#DepartmentGoalSummary.DepartmentMinDimReportingHierarchyKey,#DepartmentRevenueSummary.DepartmentMinDimReportingHierarchyKey) = #DepartmentYOYSummary.DepartmentMinDimReportingHierarchyKey
   AND ISNULL(#DepartmentGoalSummary.GoalMonthNumber,#DepartmentRevenueSummary.RevenueMonth) = #DepartmentYOYSummary.RevenueMonth
 GROUP BY ISNULL(#DepartmentGoalSummary.DimClubKey,#DepartmentRevenueSummary.DimClubkey),
          ISNULL(#DepartmentGoalSummary.RevenueReportingDepartment,#DepartmentRevenueSummary.RevenueReportingDepartment)
 


--Result set         
SELECT RevenueProductGroup RowLabel,
       'Actual Revenue:' CategoryActual,
       ...
       NULL AS RevenueReportingDepartmentNameCommaList,   -----@RevenueReportingDepartmentNameCommaList RevenueReportingDepartmentNameCommaList,   ---- must create within Cognos
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName+'-'+RevenueReportingDepartment+'-'+RevenueProductGroup ReportSort,
       'N' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
       NULL AS HeaderSubdivisionList    -------- @HeaderSubdivisionList HeaderSubdivisionList   ---- must create within Cognos
FROM #ProductGroupDetailSummary         
JOIN #DimLocationInfo 
   ON #ProductGroupDetailSummary.DimClubKey = #DimLocationInfo.DimClubKey

UNION ALL

SELECT RevenueReportingDepartment + ' Totals:' RowLabel,
       'Actual Revenue:' CategoryActual,
       Cast(JanuaryActual as Decimal(10,0)) JanuaryActual,
       Cast(FebruaryActual as Decimal(10,0)) FebruaryActual,
       Cast(MarchActual as Decimal(10,0)) MarchActual,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName+'-'+RevenueReportingDepartment+'-zzz' ReportSort,
       'Y' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
...
JOIN #DimLocationInfo 
  ON #DepartmentDetailSummary.DimClubKey = #DimLocationInfo.DimClubKey      

UNION ALL

SELECT RevenueReportingDepartment RowLabel,
       NULL CategoryActual,
       NULL JanuaryActual,
       NULL FebruaryActual,
       NULL MarchActual,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName+'-'+RevenueReportingDepartment+'-000' ReportSort,
       'N' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
...
GROUP BY RevenueReportingDepartment,
         ...
         #DimLocationInfo.ClubName

UNION ALL

SELECT #DimLocationInfo.ClubName + ' Totals:' RowLabel,
       'Actual Revenue:' CategoryActual,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName+'-zzz-zzz' ReportSort,
       'Y' TotalBorderFlag,
       'Local Currency' AS CurrencyCode,
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
...
SELECT #DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName RowLabel,
       NULL CategoryActual,
       NULL JanuaryActual,
       NULL FebruaryActual,
       NULL MarchActual,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-'+#DimLocationInfo.ClubName+'-000-000' ReportSort,
       'N' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
...
SELECT #DimLocationInfo.Region + ' Totals:' RowLabel,
       'Actual Revenue:' CategoryActual,
       Cast(Sum(JanuaryActual) as Decimal(10,0)) JanuaryActual,
       Cast(Sum(FebruaryActual) as Decimal(10,0)) FebruaryActual,
       Cast(Sum(MarchActual) as Decimal(10,0)) MarchActual,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-'+#DimLocationInfo.Region+'-zzz-zzz-zzz' ReportSort,
       'Y' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
...
SELECT 'Report Totals:' RowLabel,
       ...
       @SalesSourceCommaList SalesSourceCommaList,
       @ReportRunDateTime ReportRunDateTime,
       'Report-zzz-zzz-zzz-zzz' ReportSort,
       'Y' TotalBorderFlag,
       'Local Currency' AS CurrencyCode, 
       NULL AS RevenueProductGroupNameCommaList,  ------@RevenueProductGroupNameCommaList RevenueProductGroupNameCommaList,  ---- must create within Cognos
       NULL AS HeaderDivisionList, --------@HeaderDivisionList HeaderDivisionList,   ---- must create within Cognos
