CREATE VIEW [sandbox_ebi].[v_product_detail_personal_training_division] AS SELECT 'Hybris' AS  source_system,
 ...
 WHERE (vDimHybrisProduct.dim_hybris_product_key Is Null 
		...
		AND vDimHybrisProduct.reporting_division = 'Personal Training'
...
SELECT 'HealthCheckUSA' AS  source_system,
 ...
 WHERE (vDimHealthCheckUSAProduct.dim_healthcheckusa_product_key Is Null 
		...
		AND vDimHealthCheckUSAProduct.reporting_division = 'Personal Training'
...
SELECT 'Cafe' AS source_system,
 ...
 WHERE (vDimCafeProduct.dim_cafe_product_key Is Null OR  vDimCafeProduct.dim_cafe_product_key > '0' )   ------- removes the "unknown"  (-998) product records from being returned
   AND vDimCafeProduct.reporting_division = 'Personal Training'
...
SELECT 'MMS' AS source_system,
  ...
  FROM [marketing].[v_dim_mms_product] vDimMMSProduct
 WHERE(vDimMMSProduct.dim_mms_product_key Is Null 
	  ...
	  AND (vDimMMSProduct.reporting_division = 'Personal Training'
	       OR vDimMMSProduct.workday_cost_center in(51005, 51010, 51015, 51020, 51025, 51040, 51097))
 ...
 WHERE (vDimMagentoProduct.dim_magento_product_key Is Null 
       OR  vDimMagentoProduct.dim_magento_product_key > '0' )   ------- removes the "unknown" (-998) product records from being returned
	   AND (vDimMagentoProduct.reporting_division = 'Personal Training'
	       OR vDimMagentoProduct.workday_costcenter_id in(51005, 51010, 51015, 51020, 51025, 51040, 51097));