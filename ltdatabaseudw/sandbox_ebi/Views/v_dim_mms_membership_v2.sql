CREATE VIEW [Sandbox_ebi].[v_dim_mms_membership_v2] AS SELECT
		...
		------below existing attributes in membership 
     ...
     --  Membership.dim_mms_membership_type_key dim_mms_membership_type_key, --can be key to join d_mms_membership_type_attriube dtable
       ...
       Membership.membership_type membership_type,					--Membership type ---------------------talk to susan about bringing hold: related here. 
...
JOIN	(SELECT MembershipHistory.dim_mms_membership_key [dim_mms_membership_key],
			MAX([MembershipHistory].effective_date_time) effective_date_time
		FROM [dbo].[dim_mms_membership_history] MembershipHistory
		GROUP BY MembershipHistory.Dim_mms_membership_key) A
	ON A.effective_date_time =  MembershipHistory.effective_date_time
    AND A.dim_mms_membership_key = MembershipHistory.dim_mms_membership_key
...
----------Where clause here is to filter off of the primary individual from member table
