CREATE VIEW [marketing].[v_fact_healthcheckusa_allocated_transaction_item]
AS select fact_healthcheckusa_allocated_transaction_item.allocated_dim_club_key allocated_dim_club_key,
       ...
       dim_healthcheckusa_product.dim_reporting_hierarchy_key dim_reporting_hierarchy_key,
	   ...
	   fact_healthcheckusa_allocated_transaction_item.transaction_post_dim_time_key transaction_post_dim_time_key,
  ...
  join marketing.v_dim_healthcheckusa_product_history dim_healthcheckusa_product 
    on fact_healthcheckusa_allocated_transaction_item.dim_healthcheckusa_product_key = dim_healthcheckusa_product.dim_healthcheckusa_product_key
   and dim_healthcheckusa_product.effective_date_time < fact_healthcheckusa_allocated_transaction_item.allocated_recalculate_through_datetime
   and dim_healthcheckusa_product.expiration_date_time >= fact_healthcheckusa_allocated_transaction_item.allocated_recalculate_through_datetime
   and dim_healthcheckusa_product.dim_healthcheckusa_product_key not in ('-999','-998','-997')
 where dim_healthcheckusa_product.reporting_product_group <> '';