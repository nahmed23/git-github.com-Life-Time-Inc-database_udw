CREATE PROC [dbo].[proc_dim_exerp_activity_tagging] @dv_batch_id [varchar](500) AS
BEGIN
	SET XACT_ABORT ON
	SET NOCOUNT ON
	...
	CREATE TABLE dbo.#etl_step1
		WITH (
				distribution = HASH (dim_exerp_activity_tagging_key)
				,location = user_db
				) AS
select case when d_exerp_activity.bk_hash in ('-997','-998','-999') then d_exerp_activity.bk_hash
       else convert(char(32),hashbytes('md5',('P%#&z$@k'+isnull(cast(cast(d_exerp_activity.activity_id as int) as varchar(500)),'z#@$k%&P')+
	                                          'P%#&z$@k'+isnull(cast(cast(d_boss_taggings.taggings_id as int) as varchar(500)),'z#@$k%&P'))),2) end dim_exerp_activity_tagging_key,
	    ...
	    d_exerp_activity.bk_hash dim_exerp_activity_key,
	   ...
	   CASE 
			WHEN isnull(d_boss_taggings.dv_load_date_time, 'Jan 1, 1753') >= isnull(d_boss_tags.dv_load_date_time, 'Jan 1, 1753')
				...
				AND isnull(d_boss_taggings.dv_load_date_time, 'Jan 1, 1753') >= isnull(d_exerp_activity.dv_load_date_time, 'Jan 1, 1753')
			...
			WHEN isnull(d_boss_tags.dv_load_date_time, 'Jan 1, 1753') >= isnull(d_boss_asi_invtr.dv_load_date_time, 'Jan 1, 1753')
				AND isnull(d_boss_tags.dv_load_date_time, 'Jan 1, 1753') >= isnull(d_exerp_activity.dv_load_date_time, 'Jan 1, 1753')
			    THEN isnull(d_boss_tags.dv_load_date_time, 'Jan 1, 1753')
			WHEN isnull(d_boss_asi_invtr.dv_load_date_time, 'Jan 1, 1753') >= isnull(d_exerp_activity.dv_load_date_time, 'Jan 1, 1753')
			    THEN isnull(d_boss_asi_invtr.dv_load_date_time, 'Jan 1, 1753')
			ELSE isnull(d_exerp_activity.dv_load_date_time, 'Jan 1, 1753')
		...
		,CASE 
			WHEN isnull(d_boss_taggings.dv_batch_id, - 1) >= isnull(d_boss_tags.dv_batch_id, - 1)
				...
				AND isnull(d_boss_taggings.dv_batch_id, - 1) >= isnull(d_exerp_activity.dv_batch_id, - 1)
				THEN isnull(d_boss_taggings.dv_batch_id, - 1)
			WHEN isnull(d_boss_tags.dv_batch_id, - 1) >= isnull(d_boss_asi_invtr.dv_batch_id, - 1)
				AND isnull(d_boss_tags.dv_batch_id, - 1) >= isnull(d_exerp_activity.dv_batch_id, - 1)
			    THEN isnull(d_boss_tags.dv_batch_id, - 1)
			WHEN isnull(d_boss_asi_invtr.dv_batch_id, - 1) >= isnull(d_exerp_activity.dv_batch_id, - 1)
			    THEN isnull(d_boss_asi_invtr.dv_batch_id, - 1)
			ELSE isnull(d_exerp_activity.dv_batch_id, - 1)
			END dv_batch_id
  ...
  join [dbo].[d_exerp_activity] 
    on [d_boss_asi_invtr].dim_boss_product_key = [d_exerp_activity].dim_boss_product_key
  where [d_boss_taggings].[taggable_type] = 'BOSS::Model::Product'
	...
	DELETE dbo.dim_exerp_activity_tagging
	WHERE dim_exerp_activity_tagging_key IN (
			SELECT dim_exerp_activity_tagging_key
			FROM dbo.#etl_step1
			)

	INSERT INTO dim_exerp_activity_tagging (
         dim_exerp_activity_tagging_key
	    ,tags_id
        ,tag_type
        ,tag_name
        ,taggings_id
