CREATE PROC [dbo].[proc_dim_mdm_golden_record_customer_phone_list] @dv_batch_id [varchar](500) AS
begin
...
with 
goldenrecordcustomer_phone_list (row_num, entity_id, phone_1, phone_2, dv_load_date_time, dv_load_end_date_time, dv_batch_id) as

   (select #p_mdm_golden_record_customer_insert.row_num,
           p_mdm_golden_record_customer.entity_id,
           s_mdm_golden_record_customer.phone_1, 
           s_mdm_golden_record_customer.phone_2,
     ...
     where p_mdm_golden_record_customer.dv_load_end_date_time = convert(datetime,'9999.12.31',102)
       and (s_mdm_golden_record_customer.phone_1 is not null or s_mdm_golden_record_customer.phone_2 is not null)
   )
...
select row_num,
	   ...
	   dv_batch_id
  from goldenrecordcustomer_phone_list
...
delete dbo.dim_mdm_golden_record_customer_phone_list
...
begin
    ...
    exec dbo.proc_util_sequence_number_get_next @table_name = 'dim_mdm_golden_record_customer_phone_list', @id_count = @insert_count, @start_id = @start_p_id out
...
begin tran
  ...
  insert dbo.dim_mdm_golden_record_customer_phone_list(
               dim_mdm_golden_record_customer_phone_list_key,
			   entity_id,
               phone,
			   type,
               dv_load_date_time,
