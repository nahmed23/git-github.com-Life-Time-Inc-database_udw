CREATE PROC [dbo].[proc_ClubUsageSummary_GenderAge_Region] @ReportBeginDate [DATETIME],@ReportEndDate [DATETIME],@ReportMMSRegionNameList [VARCHAR](1000) AS
BEGIN
...
SELECT CAST(region.description AS VARCHAR(50)) as RegionName,
       ...
       CAST(SUM(CASE 
                ...
                AND (FactMemberUsage.Member_Age_Years < 19 OR FactMemberUsage.Member_Age_Years IS NULL)
                     THEN 1 
                     ELSE 0 
       ...
       CAST(SUM(CASE 
                WHEN FactMemberUsage.Gender_Abbreviation = 'M' 
                AND FactMemberUsage.Member_Age_Years >= 56
                     THEN 1 
                     ELSE 0 
