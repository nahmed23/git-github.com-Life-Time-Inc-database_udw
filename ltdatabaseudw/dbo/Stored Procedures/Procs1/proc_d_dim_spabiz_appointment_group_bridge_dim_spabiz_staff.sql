CREATE PROC [dbo].[proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff] @current_dv_batch_id [bigint] AS
begin
...
--Start!
exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff','proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff start',@current_dv_batch_id

-- Get the @max_dv_batch_id from dimension/fact.  Use -2 if there aren't any records.
exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff','max dv_batch_id',@current_dv_batch_id
...
exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff','#p_spabiz_ap_group_data_insert',@current_dv_batch_id
if object_id('tempdb..#p_spabiz_ap_group_data_insert') is not null drop table #p_spabiz_ap_group_data_insert
create table dbo.#p_spabiz_ap_group_data_insert with(distribution=round_robin, location=user_db, heap) as
select p_spabiz_ap_group_data.p_spabiz_ap_group_data_id,
       p_spabiz_ap_group_data.bk_hash,
       row_number() over (order by p_spabiz_ap_group_data_id) row_num
  from dbo.p_spabiz_ap_group_data
  join #batch_id
    on p_spabiz_ap_group_data.dv_batch_id > #batch_id.max_dv_batch_id
    or p_spabiz_ap_group_data.dv_batch_id = #batch_id.current_dv_batch_id
 where p_spabiz_ap_group_data.dv_load_end_date_time = convert(datetime,'9999.12.31',102)

-- calculate all values of the records to be inserted to make the actual update go as fast as possible
exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff','#insert',@current_dv_batch_id
if object_id('tempdb..#insert') is not null drop table #insert
create table dbo.#insert with(distribution=round_robin, location=user_db, heap) as
select #p_spabiz_ap_group_data_insert.row_num,
       p_spabiz_ap_group_data.bk_hash fact_spabiz_ap_group_data_key,
  ...
  join #p_spabiz_ap_group_data_insert
    on p_spabiz_ap_group_data.p_spabiz_ap_group_data_id = #p_spabiz_ap_group_data_insert.p_spabiz_ap_group_data_id
  join dbo.l_spabiz_ap_group_data
    on p_spabiz_ap_group_data.l_spabiz_ap_group_data_id = l_spabiz_ap_group_data.l_spabiz_ap_group_data_id
  join dbo.s_spabiz_ap_group_data
    on p_spabiz_ap_group_data.s_spabiz_ap_group_data_id = s_spabiz_ap_group_data.s_spabiz_ap_group_data_id
...
begin
    ...
    exec dbo.proc_util_sequence_number_get_next @table_name = 'd_dim_spabiz_appointment_group_bridge_dim_spabiz_staff', @id_count = @insert_count, @start_id = @start_p_id out
...
--   insert records from all of the joins to the pit table and to #p_*_insert temp table
    ...
    exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff',@task_description,@current_dv_batch_id
    begin tran
      delete dbo.d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff
       where d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff.fact_spabiz_ap_group_data_key in (select bk_hash from #p_spabiz_ap_group_data_insert where row_num >= @start and row_num < @start+1000000)

      insert dbo.d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff(
                 d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff_id,
                 fact_spabiz_ap_group_data_key,
                 ap_group_data_id,
                 store_number,
...
--Done!
exec dbo.proc_util_task_status_insert 'proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff','proc_d_dim_spabiz_appointment_group_bridge_dim_spabiz_staff end',@current_dv_batch_id
end
