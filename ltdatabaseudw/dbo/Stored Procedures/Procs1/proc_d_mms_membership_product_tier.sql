CREATE PROC [dbo].[proc_d_mms_membership_product_tier] @current_dv_batch_id [bigint] AS
begin
...
declare @max_dv_batch_id bigint = (select isnull(max(dv_batch_id),-2) from d_mms_membership_product_tier)

-- Find the PIT records with dv_batch_id > @max_dv_batch_id
-- and find the PIT records with dv_batch_id = @current_dv_batch_id since those need to be deleted in case of a retry
if object_id('tempdb..#p_mms_membership_product_tier_insert') is not null drop table #p_mms_membership_product_tier_insert
create table dbo.#p_mms_membership_product_tier_insert with(distribution=hash(bk_hash), location=user_db) as
select p_mms_membership_product_tier.p_mms_membership_product_tier_id,
 ...
 where p_mms_membership_product_tier.dv_load_end_date_time = convert(datetime,'9999.12.31',102)
   and (p_mms_membership_product_tier.dv_batch_id > @max_dv_batch_id
        or p_mms_membership_product_tier.dv_batch_id = @current_dv_batch_id)

-- calculate all values of the records to be inserted to make the actual update go as fast as possible
if object_id('tempdb..#insert') is not null drop table #insert
create table dbo.#insert with(distribution=hash(bk_hash), location=user_db) as
select p_mms_membership_product_tier.bk_hash,
       ...
       case when p_mms_membership_product_tier.bk_hash in ('-997','-998','-999') then p_mms_membership_product_tier.bk_hash
            ...
            else convert(char(32),hashbytes('md5',('P%#&z$@k'+isnull(cast(l_mms_membership_product_tier.membership_id as varchar(500)),'z#@$k%&P'))),2)
        end dim_mms_membership_key,
       case when p_mms_membership_product_tier.bk_hash in ('-997','-998','-999') then p_mms_membership_product_tier.bk_hash
            when l_mms_membership_product_tier.product_tier_id is null then '-998'
            else convert(char(32),hashbytes('md5',('P%#&z$@k'+isnull(cast(l_mms_membership_product_tier.product_tier_id as varchar(500)),'z#@$k%&P'))),2)
  ...
  join #p_mms_membership_product_tier_insert
   ...
   and p_mms_membership_product_tier.p_mms_membership_product_tier_id = #p_mms_membership_product_tier_insert.p_mms_membership_product_tier_id
...
begin tran
  delete dbo.d_mms_membership_product_tier
   where d_mms_membership_product_tier.bk_hash in (select bk_hash from #p_mms_membership_product_tier_insert)

  insert dbo.d_mms_membership_product_tier(
             bk_hash,
...
--force replication
set @max_dv_batch_id = (select isnull(max(dv_batch_id),-2) from d_mms_membership_product_tier)
--Done!
end
