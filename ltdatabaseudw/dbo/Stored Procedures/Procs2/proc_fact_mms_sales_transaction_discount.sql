CREATE PROC [dbo].[proc_fact_mms_sales_transaction_discount] @dv_batch_id [varchar](500),@begin_extract_date_time [datetime] AS
begin
...
declare @max_dv_batch_id bigint = (select max(isnull(dv_batch_id,-1)) from fact_mms_sales_transaction_discount)
...
create table dbo.#etl_step_1 with(distribution=hash(fact_mms_sales_transaction_discount_key), location=user_db) as
 select tran_item_discount.bk_hash fact_mms_sales_transaction_discount_key,
 ...
 select @factSalesTransaction_3_month = month_starting_dim_date_key
  from dim_date
 where  dim_date.calendar_date =dateadd(mm,-3,convert(varchar, @begin_extract_date_time, 112)) 
...
create table dbo.#etl_step_3 with(distribution=hash(fact_mms_sales_transaction_discount_key), location=user_db) as
 select #etl_step_1.fact_mms_sales_transaction_discount_key fact_mms_sales_transaction_discount_key,
		...
		dim_mms_club_pos_pricing_discount.dim_mms_club_pos_pricing_discount_key dim_mms_club_pos_pricing_discount_key,
 ...
 left join dim_mms_club_pos_pricing_discount dim_mms_club_pos_pricing_discount
 on #etl_step_1.pricing_discount_id = dim_mms_club_pos_pricing_discount.pricing_discount_id
   
	
if object_id('tempdb..#etl_step_4') is not null drop table #etl_step_4
create table dbo.#etl_step_4 with(distribution=hash(fact_mms_sales_transaction_discount_key), location=user_db) as
 select #etl_step_1.fact_mms_sales_transaction_discount_key fact_mms_sales_transaction_discount_key,
        fact_mms_sales_transaction_item.tran_item_id tran_item_id,
        #etl_step_1.tran_item_discount_id tran_item_discount_id,
        fact_mms_sales_transaction_item.udw_inserted_dim_date_key udw_inserted_dim_date_key,
...
create table dbo.#etl_step_5 with(distribution=hash(fact_mms_sales_transaction_discount_key), location=user_db) as

select #etl_step_3.fact_mms_sales_transaction_discount_key,
...
select #etl_step_4.fact_mms_sales_transaction_discount_key,
...
--   Insert records from records from current and missing batches
  ...
  delete dbo.fact_mms_sales_transaction_discount
   where fact_mms_sales_transaction_discount_key in (select fact_mms_sales_transaction_discount_key from dbo.#etl_step_5) 

   insert into  fact_mms_sales_transaction_discount
  ...
  select fact_mms_sales_transaction_discount_key 
          ,tran_item_id 
          ,tran_item_discount_id 
          ,fact_mms_sales_transaction_edw_inserted_dim_date_key 
          ,original_currency_code 
   ...
   from #etl_step_5
	...
	-- Populate the table
truncate table dbo.fact_mms_sales_transaction_discount
exec dbo.proc_fact_mms_sales_transaction_discount -1 ,'1757-01-01 00:00:00'