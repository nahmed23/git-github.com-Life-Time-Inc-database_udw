CREATE PROC [dbo].[proc_fact_mms_membership_balance_history] @dv_batch_id [varchar](500) AS
begin

set xact_abort on
set nocount on

declare @max_dv_batch_id bigint = (select max(isnull(dv_batch_id,-1)) from fact_mms_membership_balance_history)
...
select 
     ...
     case when membership_balance_snapshot.dim_mms_membership_key in ('-997', '-998', '-999') then membership_balance_snapshot.dim_mms_membership_key
             else convert(char(32),hashbytes('md5',('P%#&z$@k'+isnull(isnull(dim_club.local_currency_code,'USD'),'z#@$k%&P')+
                                                'P%#&z$@k'+isnull('USD','z#@$k%&P'))),2)
     end usd_dim_plan_exchange_rate_key,
     membership_balance_snapshot.effective_date_time,
     membership_balance_snapshot.expiration_date_time,
...
from d_mms_membership_balance_snapshot_history membership_balance_snapshot
     left join d_mms_membership membership
     on membership_balance_snapshot.membership_id = membership.membership_id
     ...
     left join dbo.dim_club dim_club
...
where 
membership_balance_snapshot.effective_date_time !=  membership_balance_snapshot.expiration_date_time
...
begin tran
  ...
  delete dbo.fact_mms_membership_balance_history
   where dv_batch_id in (select dv_batch_id from dbo.#etl_step1) 

  insert into dbo.fact_mms_membership_balance_history
