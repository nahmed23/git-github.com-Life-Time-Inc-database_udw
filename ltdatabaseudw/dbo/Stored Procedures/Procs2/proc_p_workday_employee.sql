CREATE PROC [dbo].[proc_p_workday_employee] @current_dv_batch_id [bigint] AS
begin
...
set @wf_name = 'wf_dv_workday_employee'
...
select bk_hash,
  ...
  join #batch_id
    on p_workday_employee.dv_batch_id = #batch_id.dv_batch_id
 group by bk_hash,
       dv_load_date_time

begin tran

delete from dbo.p_workday_employee
 where dv_batch_id in (select dv_batch_id from #batch_id)

update p_workday_employee
   set dv_load_end_date_time = 'dec 31, 9999'
  from #delete
 where p_workday_employee.bk_hash = #delete.bk_hash
   and p_workday_employee.dv_load_end_date_time = #delete.dv_load_date_time
...
if object_id('tempdb..#l_workday_employee') is not null drop table #l_workday_employee
create table dbo.#l_workday_employee with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select l_workday_employee.l_workday_employee_id,
       l_workday_employee.bk_hash,
       l_workday_employee.employee_id,
       l_workday_employee.dv_load_date_time,
       l_workday_employee.dv_batch_id,
       dense_rank() over (partition by l_workday_employee.bk_hash order by l_workday_employee.dv_load_date_time) ranking
  from dbo.l_workday_employee
  join #max_batch_id
    on l_workday_employee.dv_batch_id > #max_batch_id.dv_batch_id

if object_id('tempdb..#s_workday_employee') is not null drop table #s_workday_employee
create table dbo.#s_workday_employee with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select s_workday_employee.s_workday_employee_id,
       s_workday_employee.bk_hash,
       s_workday_employee.employee_id,
       s_workday_employee.dv_load_date_time,
       s_workday_employee.dv_batch_id,
       dense_rank() over (partition by s_workday_employee.bk_hash order by s_workday_employee.dv_load_date_time) ranking
  from dbo.s_workday_employee
  join #max_batch_id
    on s_workday_employee.dv_batch_id > #max_batch_id.dv_batch_id
...
if object_id('tempdb..#l_workday_employee_end') is not null drop table #l_workday_employee_end
create table dbo.#l_workday_employee_end with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select l_workday_employee_1.l_workday_employee_id,
       l_workday_employee_1.bk_hash,
       l_workday_employee_1.dv_load_date_time,
       l_workday_employee_1.dv_batch_id,
       isnull(l_workday_employee_2.dv_load_date_time, 'Dec 31, 9999') dv_load_end_date_time
  from #l_workday_employee l_workday_employee_1
  left join #l_workday_employee l_workday_employee_2
    on l_workday_employee_1.bk_hash = l_workday_employee_2.bk_hash
   and l_workday_employee_1.ranking + 1 = l_workday_employee_2.ranking

if object_id('tempdb..#s_workday_employee_end') is not null drop table #s_workday_employee_end
create table dbo.#s_workday_employee_end with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select s_workday_employee_1.s_workday_employee_id,
       s_workday_employee_1.bk_hash,
       s_workday_employee_1.dv_load_date_time,
       s_workday_employee_1.dv_batch_id,
       isnull(s_workday_employee_2.dv_load_date_time, 'Dec 31, 9999') dv_load_end_date_time
  from #s_workday_employee s_workday_employee_1
  left join #s_workday_employee s_workday_employee_2
    on s_workday_employee_1.bk_hash = s_workday_employee_2.bk_hash
   and s_workday_employee_1.ranking + 1 = s_workday_employee_2.ranking
...
select bk_hash,
       ...
       dv_load_date_time,
       dv_batch_id
  from #l_workday_employee
union
select bk_hash,
       employee_id,
       dv_load_date_time,
       dv_batch_id
  from #s_workday_employee
...
select p_workday_employee.p_workday_employee_id,
       ...
       p_workday_employee.l_workday_employee_id,
       p_workday_employee.s_workday_employee_id,
       p_workday_employee.dv_load_end_date_time
  from dbo.p_workday_employee
  join #du
    on p_workday_employee.bk_hash = #du.bk_hash
 where p_workday_employee.dv_load_end_date_time = 'Dec 31, 9999'
...
select pr1.bk_hash,
       ...
       isnull(l_workday_employee_end.l_workday_employee_id, isnull(p_active.l_workday_employee_id,-998)) l_workday_employee_id,
       isnull(s_workday_employee_end.s_workday_employee_id, isnull(p_active.s_workday_employee_id,-998)) s_workday_employee_id,
       p_active.p_workday_employee_id,
       case when p_active.p_workday_employee_id is null and pr1.ranking = 1 then 1 else 0 end dv_first_in_key_series
  from #pr pr1
  left join #pr pr2
  ...
  left join #l_workday_employee_end l_workday_employee_end
    on pr1.bk_hash = l_workday_employee_end.bk_hash
   and pr1.dv_load_date_time >= l_workday_employee_end.dv_load_date_time
   and isnull(pr2.dv_load_date_time,'dec 31, 9999') <= l_workday_employee_end.dv_load_end_date_time
  left join #s_workday_employee_end s_workday_employee_end
    on pr1.bk_hash = s_workday_employee_end.bk_hash
   and pr1.dv_load_date_time >= s_workday_employee_end.dv_load_date_time
   and isnull(pr2.dv_load_date_time,'dec 31, 9999') <= s_workday_employee_end.dv_load_end_date_time
...
select #p_new.bk_hash,
 ...
 join dbo.l_workday_employee
   on #p_new.l_workday_employee_id = l_workday_employee.l_workday_employee_id
union all
select #p_new.bk_hash,
       #p_new.ranking,
       s_workday_employee.jan_one satellite_date_time,
       s_workday_employee.dv_batch_id
 from #p_new
 join dbo.s_workday_employee
   on #p_new.s_workday_employee_id = s_workday_employee.s_workday_employee_id
...
select #p_new.p_workday_employee_id,
       #p_new.bk_hash,
       #p_new.employee_id,
       #p_new.l_workday_employee_id,
       #p_new.s_workday_employee_id,
...
begin tran
    update dbo.p_workday_employee
       set dv_load_end_date_time = p_new.dv_load_date_time,
           ...
           dv_update_user = @user
      from #inserts p_new
     where p_workday_employee.p_workday_employee_id = p_new.p_workday_employee_id
       and p_workday_employee.bk_hash = p_new.bk_hash
       and p_new.ranking = 1

    insert into dbo.p_workday_employee(
        bk_hash,
        employee_id,
        l_workday_employee_id,
        s_workday_employee_id,
    ...
    select p1.bk_hash,
           p1.employee_id,
           p1.l_workday_employee_id,
           p1.s_workday_employee_id,
