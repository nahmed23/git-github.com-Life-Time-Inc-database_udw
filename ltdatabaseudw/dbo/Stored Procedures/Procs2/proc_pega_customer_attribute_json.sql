CREATE PROC [dbo].[proc_pega_customer_attribute_json] @dv_batch_id [varchar](500),@start [int],@end [int] AS
begin
	...
	select 
        ...
        replace(replace(replace(replace(replace(replace(ltrim(rtrim(membership_product_description)),'\','\\'),'/','\/'),'"','\"'),char(13),'\r'),char(10),'\n'),char(9),'\t')membership_product_description,
        sum_expected_value_60_months,
        replace(replace(replace(replace(replace(replace(ltrim(rtrim(primary_activity_segment_name)),'\','\\'),'/','\/'),'"','\"'),char(13),'\r'),char(10),'\n'),char(9),'\t')primary_activity_segment_name,
        replace(replace(replace(replace(replace(replace(ltrim(rtrim(term_risk_segment_name)),'\','\\'),'/','\/'),'"','\"'),char(13),'\r'),char(10),'\n'),char(9),'\t')term_risk_segment_name,
        nps_score,
        nps_survey_date,
        promotion_opt_in,
        global_opt_in
	from 
		wrk_pega_customer_attribute
	where 
	...
	select 
        ...
        interest_confidence
	from 
		wrk_pega_customer_attribute_interest
	where 
		dv_batch_id = @current_dv_batch_id
	...
	select 
        ...
        + ']' interest_list
	from 
		#wrk_pega_customer_attribute_interest ca_member_interest 
	group by 
		dim_mms_member_key

	-- Generate the json
	select 
		case when ca_member.sequence_number != @row_start then ',' else '' end
        + '{'
		...
		+ '"membership_created_date_time":' + isnull('"' + substring(convert(varchar(4000),ca_member.membership_created_date_time,120),1,16) + '"','null') + ','
        + '"membership_phone":' + isnull('"' + convert(varchar(4000),ca_member.membership_phone) + '"','null') + ','
        + '"membership_club_access":' + isnull('"' + convert(varchar(4000),ca_member.membership_club_access) + '"','null') + ','
        ...
        + '}' json_output
	from 
		#wrk_pega_customer_attribute ca_member
    left join 
		#wrk_pega_customer_attribute_interest_list ca_member_interest_list
			on ca_member.dim_mms_member_key = ca_member_interest_list.dim_mms_member_key
	order by 
		ca_member.sequence_number
