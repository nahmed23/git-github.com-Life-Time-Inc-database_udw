CREATE PROC [dbo].[proc_fact_spabiz_customer_summary] @dv_batch_id [bigint] AS
begin
...
select dim_spabiz_customer_key,
 ...
 select #ticket_item_category_totals.dim_spabiz_customer_key,
  ...
  join #ticket_item_category_totals
   ...
   and case when dt.data_type_name = 'service' then 'service' else 'non-service' end = #ticket_item_category_totals.data_type
...
truncate table fact_spabiz_customer_summary


insert into fact_spabiz_customer_summary (
dim_spabiz_customer_key,
latest_purchase_date_time,
latest_purchase_fact_spabiz_ticket_item_key,
...
select ac.dim_spabiz_customer_key,
       ...
       min(clp.ticket_item_date_time) latest_purchase_date_time,
       min(clp.fact_spabiz_ticket_item_key) latest_purchase_fact_spabiz_ticket_item_key,
       max(case when lp.category = 'body' then lp.ticket_item_date_time else null end) latest_body_ticket_item_date_time,
       max(case when lp.category = 'body' then lp.fact_spabiz_ticket_item_key else null end) latest_body_fact_spabiz_ticket_item_key,
       max(case when lp.category = 'hair' then lp.ticket_item_date_time else null end) latest_hair_ticket_item_date_time,
       max(case when lp.category = 'hair' then lp.fact_spabiz_ticket_item_key else null end) latest_hair_fact_spabiz_ticket_item_key,
       max(case when lp.category = 'skin' then lp.ticket_item_date_time else null end) latest_skin_ticket_item_date_time,
       max(case when lp.category = 'skin' then lp.fact_spabiz_ticket_item_key else null end) latest_skin_fact_spabiz_ticket_item_key,
       max(case when lp.category = 'nail' then lp.ticket_item_date_time else null end) latest_nail_ticket_item_date_time,
       max(case when lp.category = 'nail' then lp.fact_spabiz_ticket_item_key else null end) latest_nail_fact_spabiz_ticket_item_key,
       max(case when lp.category = 'medi' then lp.ticket_item_date_time else null end) latest_medi_ticket_item_date_time,
       max(case when lp.category = 'medi' then lp.fact_spabiz_ticket_item_key else null end) latest_medi_fact_spabiz_ticket_item_key,
       max(t.yearly_product_amount) yearly_product_amount,
       max(t.yearly_product_quantity) yearly_product_quantity,
       max(t.yearly_service_amount) yearly_service_amount,
       max(t.yearly_service_quantity) yearly_service_quantity,
       max(t.yearly_body_service_amount) yearly_body_service_amount,
       max(t.yearly_body_service_quantity) yearly_body_service_quantity,
       max(t.yearly_hair_service_amount) yearly_hair_service_amount,
       max(t.yearly_hair_service_quantity) yearly_hair_service_quantity,
       max(t.yearly_skin_service_amount) yearly_skin_service_amount,
       max(t.yearly_skin_service_quantity) yearly_skin_service_quantity,
       max(t.yearly_nail_service_amount) yearly_nail_service_amount,
       max(t.yearly_nail_service_quantity) yearly_nail_service_quantity,
       max(t.yearly_medi_service_amount) yearly_medi_service_amount,
       max(t.yearly_medi_service_quantity) yearly_medi_service_quantity,
       max(cnap.start_date_time) next_appointment_date_time,
       max(cnap.fact_spabiz_appointment_item_key) next_appointment_fact_spabiz_appointment_item_key,
       max(case when nap.category = 'body' then nap.start_date_time else null end) next_body_appointment_date_time,
       max(case when nap.category = 'body' then nap.fact_spabiz_appointment_item_key else null end) next_body_fact_spabiz_appointment_item_key,
       max(case when nap.category = 'hair' then nap.start_date_time else null end) next_hair_appointment_date_time,
       max(case when nap.category = 'hair' then nap.fact_spabiz_appointment_item_key else null end) next_hair_fact_spabiz_appointment_item_key,
       max(case when nap.category = 'skin' then nap.start_date_time else null end) next_skin_appointment_date_time,
       max(case when nap.category = 'skin' then nap.fact_spabiz_appointment_item_key else null end) next_skin_fact_spabiz_appointment_item_key,
       max(case when nap.category = 'nail' then nap.start_date_time else null end) next_nail_appointment_date_time,
       max(case when nap.category = 'nail' then nap.fact_spabiz_appointment_item_key else null end) next_nail_fact_spabiz_appointment_item_key,
       max(case when nap.category = 'medi' then nap.start_date_time else null end) next_medi_appointment_date_time,
       max(case when nap.category = 'medi' then nap.fact_spabiz_appointment_item_key else null end) next_medi_fact_spabiz_appointment_item_key,
       getdate(),
       'Dec 31, 9999',
       @dv_batch_id,
       getdate(),
