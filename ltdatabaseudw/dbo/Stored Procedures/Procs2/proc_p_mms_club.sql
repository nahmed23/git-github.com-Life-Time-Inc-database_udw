CREATE PROC [dbo].[proc_p_mms_club] @current_dv_batch_id [bigint] AS
begin
...
select bk_hash,
  ...
  from dbo.p_mms_club
  join #batch_id
    on p_mms_club.dv_batch_id = #batch_id.dv_batch_id
 group by bk_hash,
       dv_load_date_time

begin tran

delete from dbo.p_mms_club
 where dv_batch_id in (select dv_batch_id from #batch_id)

update p_mms_club
   set dv_load_end_date_time = 'dec 31, 9999'
  from #delete
 where p_mms_club.bk_hash = #delete.bk_hash
   and p_mms_club.dv_load_end_date_time = #delete.dv_load_date_time
...
if object_id('tempdb..#l_mms_club') is not null drop table #l_mms_club
create table dbo.#l_mms_club with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select l_mms_club.l_mms_club_id,
       l_mms_club.bk_hash,
       l_mms_club.club_id,
       l_mms_club.dv_load_date_time,
       l_mms_club.dv_batch_id,
       dense_rank() over (partition by l_mms_club.bk_hash order by l_mms_club.dv_load_date_time) ranking
  from dbo.l_mms_club
  join #max_batch_id
    on l_mms_club.dv_batch_id > #max_batch_id.dv_batch_id

if object_id('tempdb..#s_mms_club') is not null drop table #s_mms_club
create table dbo.#s_mms_club with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select s_mms_club.s_mms_club_id,
       s_mms_club.bk_hash,
       s_mms_club.club_id,
       s_mms_club.dv_load_date_time,
       s_mms_club.dv_batch_id,
       dense_rank() over (partition by s_mms_club.bk_hash order by s_mms_club.dv_load_date_time) ranking
  from dbo.s_mms_club
  join #max_batch_id
    on s_mms_club.dv_batch_id > #max_batch_id.dv_batch_id
...
if object_id('tempdb..#l_mms_club_end') is not null drop table #l_mms_club_end
create table dbo.#l_mms_club_end with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select l_mms_club_1.l_mms_club_id,
       l_mms_club_1.bk_hash,
       l_mms_club_1.dv_load_date_time,
       l_mms_club_1.dv_batch_id,
       isnull(l_mms_club_2.dv_load_date_time, 'Dec 31, 9999') dv_load_end_date_time
  from #l_mms_club l_mms_club_1
  left join #l_mms_club l_mms_club_2
    on l_mms_club_1.bk_hash = l_mms_club_2.bk_hash
   and l_mms_club_1.ranking + 1 = l_mms_club_2.ranking

if object_id('tempdb..#s_mms_club_end') is not null drop table #s_mms_club_end
create table dbo.#s_mms_club_end with(distribution=hash(bk_hash), location=user_db, clustered index (bk_hash)) as
select s_mms_club_1.s_mms_club_id,
       s_mms_club_1.bk_hash,
       s_mms_club_1.dv_load_date_time,
       s_mms_club_1.dv_batch_id,
       isnull(s_mms_club_2.dv_load_date_time, 'Dec 31, 9999') dv_load_end_date_time
  from #s_mms_club s_mms_club_1
  left join #s_mms_club s_mms_club_2
    on s_mms_club_1.bk_hash = s_mms_club_2.bk_hash
   and s_mms_club_1.ranking + 1 = s_mms_club_2.ranking
...
select p_mms_club.p_mms_club_id,
       p_mms_club.bk_hash,
       p_mms_club.l_mms_club_id,
       p_mms_club.s_mms_club_id,
       p_mms_club.dv_load_end_date_time
  from dbo.p_mms_club
  join #du
    on p_mms_club.bk_hash = #du.bk_hash
 where p_mms_club.dv_load_end_date_time = 'Dec 31, 9999'
...
select pr1.bk_hash,
       ...
       isnull(l_mms_club_end.l_mms_club_id, isnull(p_active.l_mms_club_id,-998)) l_mms_club_id,
       isnull(s_mms_club_end.s_mms_club_id, isnull(p_active.s_mms_club_id,-998)) s_mms_club_id,
       p_active.p_mms_club_id,
       case when p_active.p_mms_club_id is null and pr1.ranking = 1 then 1 else 0 end dv_first_in_key_series
  from #pr pr1
  left join #pr pr2
  ...
  left join #l_mms_club_end l_mms_club_end
    on pr1.bk_hash = l_mms_club_end.bk_hash
   and pr1.dv_load_date_time >= l_mms_club_end.dv_load_date_time
   and isnull(pr2.dv_load_date_time,'dec 31, 9999') <= l_mms_club_end.dv_load_end_date_time
  left join #s_mms_club_end s_mms_club_end
    on pr1.bk_hash = s_mms_club_end.bk_hash
   and pr1.dv_load_date_time >= s_mms_club_end.dv_load_date_time
   and isnull(pr2.dv_load_date_time,'dec 31, 9999') <= s_mms_club_end.dv_load_end_date_time
...
select #p_new.bk_hash,
 ...
 from #p_new
 join dbo.l_mms_club
   on #p_new.l_mms_club_id = l_mms_club.l_mms_club_id
union all
select #p_new.bk_hash,
       #p_new.ranking,
       s_mms_club.inserted_date_time satellite_date_time,
       s_mms_club.dv_batch_id
 from #p_new
 join dbo.s_mms_club
   on #p_new.s_mms_club_id = s_mms_club.s_mms_club_id
 where #p_new.dv_first_in_key_series = 1
union all
select #p_new.bk_hash,
       #p_new.ranking,
       s_mms_club.updated_date_time satellite_date_time,
       s_mms_club.dv_batch_id
 from #p_new
 join dbo.s_mms_club
   on #p_new.s_mms_club_id = s_mms_club.s_mms_club_id
...
begin tran
    update dbo.p_mms_club
       set dv_load_end_date_time = p_new.dv_load_date_time,
           ...
           dv_update_user = @user
      from #inserts p_new
     where p_mms_club.p_mms_club_id = p_new.p_mms_club_id
       and p_mms_club.bk_hash = p_new.bk_hash
       and p_new.ranking = 1

    insert into dbo.p_mms_club(
        bk_hash,
        club_id,
        l_mms_club_id,
        s_mms_club_id,
