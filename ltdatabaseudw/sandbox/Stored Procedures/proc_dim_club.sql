CREATE PROC [sandbox].[proc_dim_club] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT d_mms_club.[club_id]
  ...
  WHERE ( batch_info.[dv_batch_id] >= CASE WHEN @min_batch_id = -998 THEN -1 ELSE @min_batch_id END
      AND batch_info.[dv_batch_id] <= CASE WHEN @max_batch_id = -998 THEN 9223372036854775807 ELSE @max_batch_id END )
