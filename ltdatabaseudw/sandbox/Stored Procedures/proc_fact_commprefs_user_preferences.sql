CREATE PROC [sandbox].[proc_fact_commprefs_user_preferences] @dv_batch_id [varchar](500) AS
begin
/*
EXEC sandbox.proc_fact_commprefs_user_preferences @dv_batch_id = '20200516070109' --'-1'
*/
set xact_abort on
set nocount on
...
BEGIN
	...
	from dbo.d_mms_email_address_status  (nolock)
	...
	union
	select lower(h.email_address) as email_address, @load_dv_batch_id as load_dv_batch_id
	from dbo.h_exacttarget_open h (nolock)
	  join dbo.s_exacttarget_open s (nolock)
		on h.bk_hash = s.bk_hash
	where email_address is not null
		and s.dv_batch_id = @load_dv_batch_id 
	union
	select lower(email_address) as email_address, @load_dv_batch_id as load_dv_batch_id
	from marketing.v_fact_exacttarget_sent (nolock)
	where email_address is not null
	and dv_batch_id = @load_dv_batch_id 
	union
	select lower(email_address) as email_address, @load_dv_batch_id as load_dv_batch_id
	from marketing.v_fact_exacttarget_bounces (nolock)
	where email_address is not null
	and dv_batch_id = @load_dv_batch_id 
	union
	select lower(email_address) as email_address, @load_dv_batch_id as load_dv_batch_id
	from marketing.v_fact_exacttarget_clicks (nolock)
	where email_address is not null
	and dv_batch_id = @load_dv_batch_id 
	union
	...
	from d_commprefs_communication_values (nolock)
	left join d_commprefs_party_communication_values (nolock)
	...
	left join d_commprefs_membership_segment_parties (nolock)
	...
	left join d_commprefs_communication_type_channel_membership_segments (nolock)
	...
	left join d_commprefs_communication_type_channels (nolock)
	...
	left join d_commprefs_communication_types (nolock)
	...
	left join d_commprefs_communication_preferences (nolock)
	...
	(select dim_commprefs_membership_segments_key from d_commprefs_membership_segments  (nolock) where membership_segments_key_value='non_member')
	left join d_commprefs_communication_type_channels (nolock)
	on d_commprefs_communication_type_channel_membership_segments.d_commprefs_communication_type_channels_bk_hash=d_commprefs_communication_type_channels.d_commprefs_communication_type_channels_key
	left join d_commprefs_communication_types (nolock)
	on d_commprefs_communication_type_channels.d_commprefs_communication_types_bk_hash = d_commprefs_communication_types.bk_hash
...
begin tran
  ...
  --delete dbo.fact_commprefs_user_preferences  --RJ >> USE THIS WHEN DEPLOYING TO UDW
   delete sandbox.UAT_fact_commprefs_user_preferences  --RJ >> REMOVE THIS WHEN DEPLOYING TO UDW
   where email_address in (select email_address from dbo.#etl_step_4)

    --insert into fact_commprefs_user_preferences  --RJ >> USE THIS WHEN DEPLOYING TO UDW
	insert into sandbox.UAT_fact_commprefs_user_preferences  --RJ >> REMOVE THIS WHEN DEPLOYING TO UDW
	(
	  ae_last_sent_date,
      ae_total_sent,
      el_last_sent_date,
