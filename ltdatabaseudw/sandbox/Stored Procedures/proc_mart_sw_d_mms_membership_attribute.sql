CREATE PROC [sandbox].[proc_mart_sw_d_mms_membership_attribute] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT PIT.[membership_attribute_id]
     , LNK.[membership_id]
  ...
  FROM [dbo].[p_mms_membership_attribute] PIT
       ...
       INNER JOIN
         ( SELECT PIT.[p_mms_membership_attribute_id]
             ...
             FROM [dbo].[p_mms_membership_attribute] PIT
                  INNER JOIN
                    ( SELECT HUB.[bk_hash]
                           ...
                           , HUB.[dv_deleted]
                        FROM [dbo].[h_mms_membership_attribute] HUB
                    ) HUB
         ...
         ON PITU.[p_mms_membership_attribute_id] = PIT.[p_mms_membership_attribute_id]
            AND ((PITU.[dv_deleted] = 0 AND PITU.RowRank = 1 AND PITU.RowNumber = 1) OR PITU.[dv_deleted] = 1)
  WHERE NOT PIT.[membership_attribute_id] Is Null
