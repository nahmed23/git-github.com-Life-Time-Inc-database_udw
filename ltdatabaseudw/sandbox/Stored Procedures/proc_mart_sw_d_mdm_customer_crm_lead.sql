CREATE PROC [sandbox].[proc_mart_sw_d_mdm_customer_crm_lead] @min_batch_id [bigint],@max_batch_id [bigint] AS
...
SELECT DIM.[dim_mdm_golden_record_customer_id_list_id]
     ...
     , [lead_id] = CONVERT(varchar(36), DIM.[id])
     , [dim_crm_lead_key] = CASE WHEN NOT DIM.[id] Is Null THEN CONVERT(char(32), HASHBYTES('MD5', ('P%#&z$@k' + CONVERT(varchar(36), DIM.[id]))),2) ELSE CONVERT(char(32),'-998',2) END
