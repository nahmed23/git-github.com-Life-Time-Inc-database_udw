CREATE PROC [sandbox].[proc_mart_sw_d_mms_membership_sales_promotion_code] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT PIT.[membership_sales_promotion_code_id]
     , LNK.[membership_id]
     , LNK.[member_id]
  ...
  FROM [dbo].[p_mms_membership_sales_promotion_code] PIT
       ...
       INNER JOIN
         ( SELECT PIT.[p_mms_membership_sales_promotion_code_id]
         ...
         ON PITU.[p_mms_membership_sales_promotion_code_id] = PIT.[p_mms_membership_sales_promotion_code_id]
            AND PITU.RowRank = 1 AND PITU.RowNumber = 1
  WHERE NOT PIT.[membership_sales_promotion_code_id] Is Null
