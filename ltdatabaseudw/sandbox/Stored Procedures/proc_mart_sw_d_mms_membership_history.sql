CREATE PROC [sandbox].[proc_mart_sw_d_mms_membership_history] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT d_mms_mbrs_history.[membership_id]
  ...
  WHERE NOT d_mms_mbrs_history.[membership_id] Is Null
    AND d_mms_mbrs_history.[membership_type_id] <> 134  --House Account
    --AND (d_mms_club_history.[d_mms_club_history_id] Is Null OR (d_mms_club_history.RowRank = 1 AND d_mms_club_history.RowNumber = 1))
    AND (d_mms_club_prod_history.[d_mms_club_product_history_id] Is Null OR (d_mms_club_prod_history.RowRank = 1 AND d_mms_club_prod_history.RowNumber = 1))
    --AND ( d_mms_mbrs_history.[dv_batch_id] >= CASE WHEN @min_batch_id = -998 THEN -1 ELSE @min_batch_id END
    --  AND d_mms_mbrs_history.[dv_batch_id] <= CASE WHEN @max_batch_id = -998 THEN 9223372036854775807 ELSE @max_batch_id END )
ORDER BY d_mms_mbrs_history.[dv_batch_id] ASC, d_mms_mbrs_history.[effective_date_time] ASC;
