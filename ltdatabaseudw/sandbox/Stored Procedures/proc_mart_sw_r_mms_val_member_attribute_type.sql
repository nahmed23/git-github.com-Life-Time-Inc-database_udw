CREATE PROC [sandbox].[proc_mart_sw_r_mms_val_member_attribute_type] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT [val_member_attribute_type_id]
     , [description]
     , [sort_order]
     , [inserted_date_time]
     , [updated_date_time]
     , [bk_hash]
     , [r_mms_val_member_attribute_type_id]
     , [dv_load_date_time]
     , [dv_batch_id]
     , [dv_hash]
  FROM [dbo].[r_mms_val_member_attribute_type]
  WHERE [dv_load_end_date_time] = '9999-12-31 00:00:00.000'
    AND NOT [val_member_attribute_type_id] Is Null
