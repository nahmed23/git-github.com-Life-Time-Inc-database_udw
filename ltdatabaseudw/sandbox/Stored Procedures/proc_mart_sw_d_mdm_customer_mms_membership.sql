CREATE PROC [sandbox].[proc_mart_sw_d_mdm_customer_mms_membership] @min_batch_id [bigint],@max_batch_id [bigint] AS
...
SELECT DIM.[dim_mdm_golden_record_customer_id_list_id]
     ...
     , [membership_id] = d_batch.[membership_id]
     , [dim_mms_membership_key] = CASE WHEN NOT d_batch.[membership_id] Is Null THEN CONVERT(char(32), HASHBYTES('MD5', ('P%#&z$@k' + CONVERT(varchar, d_batch.[membership_id]))),2) ELSE CONVERT(char(32),'-998',2) END
  ...
  FROM [dbo].[dim_mdm_golden_record_customer_id_list] DIM
       INNER JOIN
         ( SELECT PIT.[dim_mdm_golden_record_customer_id_list_id]
             ...
             WHERE PIT.[id_type] = 2  --MMS Membership
       ...
       CROSS APPLY ( SELECT [membership_id] = CASE WHEN (ISNUMERIC(DIM.[id]) = 1 AND CONVERT(bigint, DIM.[id]) <= 2147483647) THEN CONVERT(int, DIM.[id]) ELSE Null END ) d_batch
