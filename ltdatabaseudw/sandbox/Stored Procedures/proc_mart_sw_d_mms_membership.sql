CREATE PROC [sandbox].[proc_mart_sw_d_mms_membership] @min_batch_id [bigint],@max_batch_id [bigint] AS

BEGIN

SELECT PIT.[membership_id]
     , LNK.[club_id]
     ...
     --, [dv_load_date_time] = CASE WHEN NOT d_mms_membership_product_tier.[membership_product_tier_id] Is Null AND d_mms_membership_product_tier.[dv_load_date_time] > PIT.[dv_load_date_time] THEN d_mms_membership_product_tier.[dv_load_date_time] ELSE PIT.[dv_load_date_time] END
     --, [dv_batch_id] = CASE WHEN NOT d_mms_membership_product_tier.[membership_product_tier_id] Is Null AND d_mms_membership_product_tier.[dv_batch_id] > PIT.[dv_batch_id] THEN d_mms_membership_product_tier.[dv_batch_id] ELSE PIT.[dv_batch_id] END
  ...
  WHERE NOT PIT.[membership_id] Is Null
    AND ( d_batch_mms_membership.[dv_batch_id] >= CASE WHEN @min_batch_id = -998 THEN -1 ELSE @min_batch_id END
      AND d_batch_mms_membership.[dv_batch_id] <= CASE WHEN @max_batch_id = -998 THEN 9223372036854775807 ELSE @max_batch_id END )
ORDER BY d_batch_mms_membership.[dv_batch_id] ASC, d_batch_mms_membership.[dv_load_date_time] ASC, ISNULL(d_batch_mms_membership.[updated_date_time], d_batch_mms_membership.[inserted_date_time]) ASC;
