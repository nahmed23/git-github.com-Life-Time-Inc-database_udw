CREATE VIEW [sandbox].[v_mart_sw_membership_change_history_to_on_hold]
AS SELECT vMCH.[membership_id]
       ...
       , [new_val_membership_type_attribute_id] = vMTA_New.[val_membership_type_attribute_id]
       , [new_membership_type_attribute_description] = vVMTA_New.[description]
       , [new_membership_type_attribute_group_description] = 'On Hold'
    ...
    FROM [sandbox].[v_mart_sw_membership_change_history] vMCH
         ...
         INNER JOIN [sandbox].[v_mart_mms_membership_type_attribute] vMTA_New
           ON vMTA_New.[membership_type_id] = vMCH.[new_membership_type_id]
         INNER JOIN [sandbox].[v_mart_mms_val_membership_type_attribute] vVMTA_New
           ON vVMTA_New.[val_membership_type_attribute_id] = vMTA_New.[val_membership_type_attribute_id]
    WHERE vMCH.[old_effective_date_time] = ( SELECT MAX(vMSH_Max.effective_date_time)
      ...
      --AND NOT EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_House_Account] vMTP WHERE vMTP.MembershipTypeID = vMCH.OldMembershipTypeID )
      AND EXISTS ( SELECT vMTP.[membership_type_id] FROM [sandbox].[v_mart_sw_membership_type_product_on_hold_eligible] vMTP WHERE vMTP.[membership_type_id] = vMCH.[old_membership_type_id] )
      --AND NOT EXISTS
      --  ( SELECT vMTA.MembershipTypeID
      --      FROM [dw].[vMembershipTypeAttribute] vMTA
      --      WHERE vMTA.MembershipTypeID = vMCH.OldMembershipTypeID
      --        AND vMTA.ValMembershipTypeAttributeID IN (29,57,58,59) )
      AND vMCH.[new_effective_date_time] = ( SELECT MAX(vMSH_Max.effective_date_time)
                                               ...
                                               WHERE vMSH_Max.[membership_id] = vMCH.[membership_id]
                                                 ...
                                                 AND vMSH_Max.[effective_date_no_time] >= DDate.[month_starting_date] )
      AND vMTA_New.[val_membership_type_attribute_id] = 57
      --AND EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_OnHold] vMTP WHERE vMTP.MembershipTypeID = vMCH.NewMembershipTypeID )
      --AND NOT EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_House_Account] vMTP WHERE vMTP.MembershipTypeID = vMCH.OldMembershipTypeID )
      --AND NOT EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_OnHold] vMTP WHERE vMTP.MembershipTypeID = vMCH.OldMembershipTypeID )
      --AND NOT EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_Non_Revenue] vMTP WHERE vMTP.MembershipTypeID = vMCH.OldMembershipTypeID )
      --AND NOT EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_Qualifying_Revenue] vMTP WHERE vMTP.MembershipTypeID = vMCH.OldMembershipTypeID )
