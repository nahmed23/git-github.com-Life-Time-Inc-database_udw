CREATE VIEW [sandbox].[v_mart_sw_membership_change_history_to_qualifying_revenue]
AS SELECT vMCH.[membership_id]
       ...
       , [new_membership_type_attribute_group_description] = 'Qualifying'
    ...
    WHERE vMCH.[old_effective_date_time] = ( SELECT MAX(vMSH_Max.effective_date_time)
      ...
      AND EXISTS
        ( SELECT vMTA.[membership_type_id]
            ...
            WHERE vMTA.[membership_type_id] = vMCH.[old_membership_type_id]
      ...
      AND vMCH.[new_effective_date_time] = ( SELECT MAX(vMSH_Max.effective_date_time)
                                               ...
                                               WHERE vMSH_Max.[membership_id] = vMCH.[membership_id]
                                                 ...
                                                 AND vMSH_Max.[effective_date_no_time] >= DDate.[month_starting_date] )
      AND vMTA_New.[val_membership_type_attribute_id] IN (58,59)
      --AND EXISTS ( SELECT vMTP.MembershipTypeID FROM [dw].[mart_vMembershipTypeProduct_Qualifying_Revenue] vMTP WHERE vMTP.MembershipTypeID = vMCH.NewMembershipTypeID )
